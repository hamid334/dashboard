...
namespace BasketWebPanel.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public JsonResult UploadImage(HttpPostedFileBase file)
        {
            ...
            {
                Request.RequestContext.HttpContext.Session.Remove("AddProfileImage");
                Request.RequestContext.HttpContext.Session.Add("AddProfileImage", Request.Files[0]);
        ...
        public async Task<ActionResult> UserProfile()
        {
            ...
            var UserId = User.GetClaimValue("UserId");
            var Role = User.GetClaimValue("AccountType");

            var response = await ApiCall.CallApi("api/User/GetProfile", User, null, true, false, null, "userId=" + UserId, "userRole=" + Role);
            if (response == null)
            {
            ...
            {
                model = response.GetValue("Result").ToObject<ProfileViewModel>();
                return View(model);
            }
        }
        ...
        public async Task<ActionResult> UserProfile(ProfileViewModel userProfile)
        {
            try
            {
                MultipartFormDataContent content;

                bool FileAttached = (Request.RequestContext.HttpContext.Session["AddProfileImage"] != null);
                ...
                byte[] fileData = null;
                var ImageFile = (HttpPostedFileWrapper)Request.RequestContext.HttpContext.Session["AddProfileImage"];
                if (FileAttached)
                {
                ...
                content.Add(new StringContent(userProfile.Id.ToString()), "UserId");
                ...
                {
                    content.Add(new StringContent(userProfile.Phone), "Phone");
                }
                else
                {
                    content.Add(new StringContent(""), "Phone");
                }

                response = await ApiCall.CallApi("api/User/UpdateCurrentProfile", User, isMultipart: true, multipartContent: content);

                if (response == null)
                {
                    return new HttpStatusCodeResult(HttpStatusCode.InternalServerError, "404");
                }
                else
                {
                    TempData["SuccessMessage"] = "The profile has been updated successfully.";
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            try
            {
                ...
                var response = await ApiCall.CallApi("api/User/WebPanelLogin", User, model);
        ...
        public async Task<ActionResult> UserLogin(LoginViewModel model)
        {
            try
            {
                ...
                var response = await ApiCall.CallApi("api/User/Login", User, model);
                ...
                {
                    var userViewModels = response.GetValue("Result").ToObject<UserViewModels>();
                    identity = new ClaimsIdentity(DefaultAuthenticationTypes.ApplicationCookie);
                    identity.AddClaim(new Claim("username", model.Email));
                    identity.AddClaim(new Claim("UserId", userViewModels.Id.ToString()));
                    identity.AddClaim(new Claim("FullName", userViewModels.FirstName + " " + userViewModels.LastName));
                    identity.AddClaim(new Claim("ProfilePictureUrl", userViewModels.ProfilePictureUrl ?? ""));
                    identity.AddClaim(new Claim(ClaimTypes.Name, model.Email));
                    identity.AddClaim(new Claim("AccountType", userViewModels.AccountType.ToString()));
                    identity.AddClaim(new Claim(ClaimTypes.Role, userViewModels.AccountType.ToString()));
                    identity.AddClaim(new Claim("access_token", userViewModels.Token.access_token));
                    identity.AddClaim(new Claim("token_type", userViewModels.Token.token_type));
                    identity.AddClaim(new Claim("expires_in", userViewModels.Token.expires_in));
                    identity.AddClaim(new Claim("refresh_token", userViewModels.Token.refresh_token));
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> AjaxUserLogin(LoginViewModel model)
        {
            try
            {
                ClaimsIdentity identity;
                ...
                if (response is Error)
                {
                    return Json(new { Message = "success", error = (response as Error).ErrorMessage });
                }
                else
                {
                    var userViewModels = response.GetValue("Result").ToObject<UserViewModels>();
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { Message = "success", error = ex.Message });
            }
        ...
        protected override void OnException(ExceptionContext filterContext)
        {
                ...
                filterContext.RequestContext.HttpContext.User.Identity.IsAuthenticated)
            {
                ...
                filterContext.Result = RedirectToAction("Index", "Home", new { area = "Dashboard" });
