...
namespace BasketWebPanel.Areas.Dashboard.Controllers
{
    ...
    public class OffersController : Controller
    {
        ...
        public ActionResult Latest()
        {
            ...
            var id = User.GetClaimValue("UserId");
            var response = AsyncHelpers.RunSync<JObject>(() => ApiCall.CallApi("api/GetLatestOffers", User, null, true, false, null, "User_Id=" + id));
            if (response is Error)
            {
                return new HttpStatusCodeResult(HttpStatusCode.InternalServerError, (response as Error).ErrorMessage);
            }
        ...
        public ActionResult Favourite()
        {
            ...
            var response = AsyncHelpers.RunSync<JObject>(() => ApiCall.CallApi("api/GetMyFavouriteOffers", User, null, true, false, null, $"User_Id={User.GetClaimValue("UserId")}"));
            if (response is Error)
            {
                return new HttpStatusCodeResult(HttpStatusCode.InternalServerError, (response as Error).ErrorMessage);
            }
            else
            {
                if (response != null)
                {
                    returnModel = response.GetValue("Result").ToObject<FavouritesViewModel>();
                }
            }
            returnModel.SetSharedData(User);
            return PartialView(returnModel);
        ...
        public JsonResult UnFavourite(int ProductId)
        {

            try
            {
                var UserId = User.GetClaimValue("UserId");
                var response = AsyncHelpers.RunSync<JObject>(() => ApiCall.CallApi("api/UnFavourite", User, null, true, false, null, "User_Id=" + UserId, "Id=" + ProductId));
